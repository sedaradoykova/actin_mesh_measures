...
class ActinImg:
    ...
    def visualise(
        self, imtype: str='original', ind: int=1, save: bool=False,
        ...
        ind : int=1
            Index of slice to visualise from z-stack (first frame is n=1).
        """
        if not isinstance(imtype, str) or not isinstance(ind, int):
            raise TypeError('type must be a string and ind must be an integer')
        if ind >= self.depth or ind <=0 :
            raise ValueError('ind must be an integer >0 and < {depth}'.format(depth=self.depth))        
    ...
    def visualise_stack(
        self, imtype: str='original', substacks: list=None, save: bool=False, fig_size: tuple=None,
        colmap: str='inferno', scale_bar: bool=True, bar_locate:str='upper left'):
        ...
        if imtype=='original':
            ...
            for n, image in enumerate(data[::1]):
                ...
                ax.set_axis_off()
                ax.text(0.66, 0.05, 'n={count}'.format(count=substacks[0]+n),color='white',transform=ax.transAxes)
