...
""" Design considerations: 
    ...
    A history of all changes implemented (methods called) are recorded in a hidden attribute. 

    TODO: 
        - [ ] Annotate steps of analysis in docstrings
        - [ ] meshwork_size and meshwork_density methods  
        - [ ] plots: replace white with a nice shade of gray
...
class ActinImg:
    ...
    def visualise(
        self, imtype: str='original', ind: int=0, save: bool=False,
        colmap: str='inferno', scale_bar: bool=True, bar_locate:str='upper left'
        ): 
        """ Visualise an image or stack slice. 
        ...
        Arguments 
        ---------
        imtype : str='original'
        ...
        if imtype=='manipulated':
            ...
            if self._projected or len(self.manipulated_stack.shape) == 2:
                plt.imshow(self.manipulated_stack, cmap=colmap)
            else: 
                plt.imshow(self.manipulated_stack[ind], cmap=colmap)        
        elif imtype=='original':
            if self.depth > 1:
                plt.imshow(self.image_stack[ind], cmap=colmap)
            else: 
        ...
        if (scale_bar and self.resolution is not None):
            scalebar = ScaleBar(self.resolution, 'nm', box_color='None', color='white', location=bar_locate) 
            plt.gca().add_artist(scalebar)
        plt.axis('off')
        if save: 
            dest = os.path.join(os.getcwd(), self.title, '.png')
            plt.imsave(dest)
        else:
            plt.show();
    ...
    def visualise_stack(
        self, imtype: str='original', save: bool=False, fig_size: tuple=None,
        colmap: str='inferno', scale_bar: bool=True, bar_locate:str='upper left'):
        ...
        if imtype=='original':
            figrows, figcols, left_to_del = get_fig_dims(self.depth) 
            fig, axes = plt.subplots(nrows=figrows, ncols=figcols, figsize=fig_size)
            plt_count = 1
            for ax, image in zip(axes.flatten(), self.image_stack[::1]):
                ax.imshow(image, cmap=colmap)
                if (scale_bar and self.resolution is not None):
                    scalebar = ScaleBar(self.resolution, 'nm', box_color='None', color='white', location=bar_locate) 
                    ax.add_artist(scalebar)
                ax.set_axis_off()
                ax.text(0.66, 0.05, 'n={count}'.format(count=plt_count), color='white',  transform=ax.transAxes)
                plt_count += 1
            ...
            if self._projected:
                ...
                manipulated_depth = self.manipulated_stack.shape[0]
                figrows, figcols, left_to_del = get_fig_dims(manipulated_depth) 
                fig, axes = plt.subplots(nrows=figrows, ncols=figcols, figsize=fig_size)
                plt_count = 1
                for ax, image in zip(axes.flatten(), self.manipulated_stack[::1]):
                    ax.imshow(image, cmap=colmap)
                    if (scale_bar and self.resolution is not None):
                        scalebar = ScaleBar(self.resolution, 'nm', box_color='None', color='white', location=bar_locate) 
                        ax.add_artist(scalebar)
    ...
    def normalise(self): 
        """ Normalises a z-stack (or image) by minimum pixel intensity per frame. 
        """
        if self.depth == 1: 
    ...
    def z_project_min(self, substacks: list[int]=None):
        ...
        if substacks: 
            data = self.manipulated_stack[substacks[0]:substacks[1]+1].copy() 
        else:
    ...
    def steerable_gauss_2order(self, substacks: list=None, sigma: float=2.0, theta: float=0., visualise: bool=True):
        ...
        if self.depth > 1: 
            ...
            depth, rows, cols = self.depth, *self.shape
...
def get_ActinImg(image_name: str, image_dir: str):
    ...
    return ActinImg(img, title, shape, depth, devonv, resolut, meta)
