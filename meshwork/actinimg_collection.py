    ...
    def visualise_html(self, subdir):
        ...
        with open(os.path.join(self.res_path,(subdir[0:10]+".md"), "w")) as f:
            for i in range(len(curr_filenames)):
                ...
                f.write('\n\n')
                f.write(f'![]({self.__save_destdir}/main/{all_output_types["original"][i]})'+'{ width=700px } ')
                f.write('\n\n')
                f.write('**Maximum z-projection**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/main/{all_output_types["max_proj"][i]})'+'{ height=300px }  ')
                f.write('\n\n')
                f.write('## Basal network  ')
                f.write('\n\n')
                f.write('**Steerable second order Gaussian filter**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/basal/{all_output_types["steer_gauss"][i]})'+'{ height=300px }  ')
                f.write('\n')
                f.write('**Minimum z-projection**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/basal/{all_output_types["min_proj"][i]})'+'{ height=300px }  ')
                f.write('\n')
                f.write('**Binary thresholding**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/basal/{all_output_types["threshold"][i]})'+'{ height=300px }  ')
                f.write('\n\n')
                f.write('## Cytosolic network  ')
                f.write('\n\n')
                f.write('**Steerable second order Gaussian filter**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/cytosolic/{all_output_types["steer_gauss"][i]})'+'{ height=300px }  ')
                f.write('\n')
                f.write('**Minimum z-projection**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/cytosolic/{all_output_types["min_proj"][i]})'+'{ height=300px }  ')
                f.write('\n')
                f.write('**Binary thresholding**  ')
                f.write('\n')
                f.write(f'![]({self.__save_destdir}/cytosolic/{all_output_types["threshold"][i]})'+'{ height=300px }  ')
                f.write('\n\n\n')
